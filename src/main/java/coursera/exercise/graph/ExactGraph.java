...
import static coursera.exercise.GraphUtil.computeE;
...
public class ExactGraph {
	 ...
	 */
	public ExactGraph(int V) {
		this(V, newAdj(V));
	}

	public ExactGraph(int V, Bag<Integer>[] bags) {
		if(V < 0) throw new IllegalArgumentException("Number of vertices must be nonnegative");
		this.V = V;
		adj = bags;
		this.E = computeE(adj) / 2;
	}
	 ...
	 */
	public ExactGraph(ExactGraph G) {
		this(G.V());
		this.E = G.E();
		for(int v = 0; v < G.V(); v++) {
